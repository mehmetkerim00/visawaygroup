 ...
 *
 * Два письма: о блокировке входа после пяти неудачных попыток и о
 * каждой публикации.
 *
...
async function published({ who, count, list }) {
  return send({
    subject: `Панель VisaWay: опубликовано записей — ${count}`,
    text: [
      "В панели нажали «Опубликовать». Сайт пересобирается.",
      "",
      `Кто: ${who}`,
      `Время: ${new Date().toLocaleString("ru-RU", { timeZone: "Asia/Ashgabat" })} (Ашхабад)`,
      "",
      ...
      ...list.map(x => "  · " + x),
      "",
      "Если сборка не пройдёт проверку, сайт не заменится: посетители",
      "продолжат видеть прежнюю версию."
    ].join("\n")
  });
}

module.exports = { send, lockedOut, published, configured };
